...
**`set_tf_seed()` is inert.** `R/inference_class.R:88`:
...
mention is a commented-out `# REFACTOR: check_rng_seed(...)` at
`R/calculate.R:199`. The comment above it describes TF1 semantics — build a
graph, then run a session — which is why setting a seed "before definition of a
random tensor" was once meaningful.

**And greta never passes a seed to `sample_chain()`.** The call passes
...
1. Have `set_tf_seed()` call `tf$random$set_seed()`. Smallest change, and the
   measurement above says it is sufficient for reproducibility.
2. Or thread a stateless seed through to `sample_chain(seed = )`. More work, but
