Part of #745. Retires 28 of the 38 `TF1/2` code markers, one at a time.
...
| `dag_class.R` (2) | inside `define_free_state()`, which is dead and awaiting a delete-or-file decision |
| `sampler_class.R` (2) | understood and kept, tied to #547 |
| `optimiser_class.R` (2) | the R-side optimiser loop, needs its own issue |
| `greta_model_class.R` (1) | `model(compile = TRUE)` is inert, see #833 |
| `chol2symm.R` (1) | trivial cleanup, to land with others |

## Testing

`devtools::test()` at every step: `FAIL 0 | WARN 0 | SKIP 2 | PASS 1952`,
